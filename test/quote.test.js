...
var RECIPROCAL_PROJECT_ID = RECIPROCAL_PACKAGE.licensezero[0].license.projectID
  ...
  helper(function (tmp, run, rm) {
    runSeries(
      [
        ...
        function (done) {
          run(['quote'], function (status, stdout, stderr) {
            test.equal(status, 0, 'exit 0')
            test.equal(
              stdout,
              'No License Zero dependencies found.\n',
              'none found'
            )
  ...
  helper(function (tmp, run, rm) {
    runSeries(
      [
        ...
        function (done) {
          run(['quote', '--no-reciprocal'], function (status, stdout, stderr) {
            test.equal(status, 0, 'exit 0')
            test.equal(
              stdout,
              [
  ...
  helper(function (tmp, run, rm) {
    runSeries(
      [
          ...
          runSeries(
            [
              ...
              fs.writeFile.bind(
                null,
                file,
                JSON.stringify(NONCOMMERCIAL_PACKAGE)
              )
            ],
            done
          )
        ...
        function (done) {
          run(['quote', '--no-noncommercial'], function (
            status,
            stdout,
            stderr
          ) {
